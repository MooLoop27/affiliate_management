...
@section('header', 'Edit Transaction: ' . $transaction->transaction_code)
            ...
            <form action="{{ route('transactions.update', $transaction) }}" method="POST" id="transactionForm">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="company_balance_amount" id="company_balance_amount" class="form-control @error('company_balance_amount') is-invalid @enderror" value="{{ old('company_balance_amount', $transaction->company_balance_amount) }}" step="0.01" required oninput="calculateCommissions()">
                        </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            <input type="number" name="sg_commission_percentage" id="sg_commission_percentage" class="form-control" value="{{ old('sg_commission_percentage', $transaction->sg_commission_percentage) }}" step="0.01" oninput="calculateCommissions()">
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            <input type="number" name="leader_commission_percentage" id="leader_commission_percentage" class="form-control" value="{{ old('leader_commission_percentage', $transaction->leader_commission_percentage) }}" step="0.01" oninput="calculateCommissions()">
                            <span class="input-group-text">%</span>
                        </div>
                    </div>
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div id="recipients-container">
                        @foreach($transaction->commissionDetails as $index => $detail)
                        <div class="recipient-row row g-2 mb-2 p-3 border rounded">
                            <div class="col-md-5">
                                ...
                                <select name="recipients[{{ $index }}][recipient_id]" class="form-select form-select-sm" required>
                                    ...
                                    <option value="{{ $rcp->id }}" {{ $detail->recipient_id == $rcp->id ? 'selected' : '' }}>{{ $rcp->recipient_code }} - {{ $rcp->recipient_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <div class="input-group input-group-sm">
                                    <input type="number" name="recipients[{{ $index }}][commission_percentage]" class="form-control commission-percentage" value="{{ $detail->commission_percentage }}" step="0.01" required oninput="calculateCommissions()">
                                    <span class="input-group-text">%</span>
                                </div>
                            </div>
                            <div class="col-md-3">
                                ...
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text">Rp</span>
                                    <input type="text" class="form-control commission-amount" value="Rp {{ number_format($detail->commission_amount, 0, ',', '.') }}" readonly>
                                </div>
                            </div>
...
@push('scripts')
<script>
let recipientIndex = {{ $transaction->commissionDetails->count() }};
